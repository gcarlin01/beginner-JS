// SNOB'N'US

// String
// Number
// Boolean
// Null
// Undefined
// Symbol

// Object

//STRINGS

// console.log("it works");

// const name = "gc";
// const middle = "is";
// const last = `me`;
// // "\"" backslash to get next character out of the string sintax

// // interpolation = putting a variable inside of a string
// // Concatination = 2 or more strings are combined into one

// //Option 1
// const hello = "hello my name is " + name + ". Nice to meet you";
// //              concatenation      interpolation

// //Option 2
// let hi = "hello my name is ";
// hi = hi + name;
// hi = hi + ". Nice to meet you";

// //Option 3 (recommended) ${} only with ``
// const howdy = `hello my name is ${name}. Nice to meet you. I am ${
//   1 + 35
// } years old`;

// const html = `
// <div>
//     <h2>${name}</h2>
//     <p>${howdy}</p>
// </div>
//     `;
// document.body.innerHTML = html;

// console.log(html);

// NUMBERS
// const age = 36; // whole number
// const name = "gc";
// typeof (in console) will give you the type
// const age = 36.5 FLOAT number (has decimal)
// a + b SUM
// a - b substraction
// a * b Multiplication
// a / b division
// a ** b to the power of
// Math. = math helper methods in console
// example: Math.round(25.6), Math.floor(25.6), Math.random ()

// const smarties = 20;
// const kids = 3;

// const eachKidGets = Math.floor(smarties / kids);
// const daddyGetsTheRest = smarties % kids; // % is mod (modular) which means residual after a division
// console.log(`Each kid gets ${eachKidGets} smarties`);

// NaN = not a number

// OBJECTS = groups of data
// const person = {
//   first: `giancarlo`,
//   last: `carlin`,
//   age: 36,
// };
// person.first or person.last or person.age

// Undefined = when calling a variable which has been created but has no set value
// Null = you can set a object param to null, person.middleName = null

// BOOLEAN
// something that can be det to true or false
// === best practice (trple equals) checks for value and type to be the same
